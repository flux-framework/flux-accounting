...
test_expect_success 'create valid flux-accounting DB with a proper hierarchy' '
	...
	${PYTHON} ${ACCT_CLI} add-bank A 1 &&
	${PYTHON} ${ACCT_CLI} add-bank --parent-bank=A B 1 &&
	${PYTHON} ${ACCT_CLI} add-bank --parent-bank=B D 1 &&
	${PYTHON} ${ACCT_CLI} add-bank --parent-bank=B E 1 &&
	${PYTHON} ${ACCT_CLI} add-bank --parent-bank=A C 1 &&
	${PYTHON} ${ACCT_CLI} add-bank --parent-bank=C F 1 &&
	${PYTHON} ${ACCT_CLI} add-bank --parent-bank=C G 1 &&
	${PYTHON} ${ACCT_CLI} add-user --username=user1 --account=D &&
	${PYTHON} ${ACCT_CLI} add-user --username=user2 --account=F &&
	${PYTHON} ${ACCT_CLI} add-user --username=user3 --account=F &&
	${PYTHON} ${ACCT_CLI} add-user --username=user4 --account=G
'

test_expect_success 'create hierarchy output from Python' '
	python3 ${ACCT_CLI} print-hierarchy > py-output.txt
'
