...
test_description='test priority plugin removing dependencies from jobs after updates'
...
test_expect_success 'update resource limits for the association; update plugin' '
	...
	grep "\"max_nodes\": 2147483647" user1_resource_limits.out &&
	grep "\"max_cores\": 2147483647" user1_resource_limits.out &&
	flux account-priority-update -p ${DB}
'
...
test_expect_success 'update max nodes per-association limit for queue; update DB' '
	...
	grep "\"max_nodes_per_assoc\": 2147483647" bronze_max_nodes_limit.out &&
	flux account-priority-update -p ${DB}
'
