...
test_description='test printing the DB hierarchy with some job usage ties'

. `dirname $0`/sharness.sh
SMALL_TIE=$(pwd)/small_tie.db
...
test_expect_success 'start flux-accounting service on small_no_tie DB' '
	flux account-service -p ${SMALL_TIE} -t
'
...
test_expect_success 'update usage and fair-share for the users/banks' '
	flux python ${UPDATE_USAGE} ${SMALL_TIE} leaf.1.1 100 &&
	flux python ${UPDATE_USAGE} ${SMALL_TIE} leaf.1.2 10 &&
	flux python ${UPDATE_USAGE} ${SMALL_TIE} leaf.1.3 10 &&
	flux python ${UPDATE_USAGE} ${SMALL_TIE} leaf.2.1 10 &&
...
test_expect_success 'view database hierarchy' '
	flux account view-bank -t root > small_tie.test &&
