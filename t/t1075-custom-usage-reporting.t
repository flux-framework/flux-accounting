...
test_expect_success '--start before all jobs calculates total usage' '
	flux account view-usage-report -s 24/12/01 > usage.test1 &&
	grep "TOTAL" usage.test1 &&
	grep "1260" usage.test1
'

test_expect_success '--start in middle of jobs calculates some usage' '
	flux account view-usage-report -s 25/06/01 > usage.test2 &&
	grep "TOTAL" usage.test2 &&
	grep "600" usage.test2
'

test_expect_success '--start after all jobs reports no usage' '
	flux account view-usage-report -s 99/01/01 > usage.test3 &&
	grep "TOTAL" usage.test3 &&
	grep "0.00" usage.test3
'

test_expect_success '-u: u1 has total usage of 540' '
	flux account view-usage-report -s 24/12/01 -u u1 > usage.test4 &&
	grep "540" usage.test4
'

test_expect_success '-u: u2 has total usage of 420' '
	flux account view-usage-report -s 24/12/01 -u u2 > usage.test5 &&
	grep "420" usage.test5
'

test_expect_success '-u: u3 has total usage of 300' '
	flux account view-usage-report -s 24/12/01 -u u3 > usage.test6 &&
	grep "300" usage.test6
'

test_expect_success '-b: bank A has total usage of 960' '
	flux account view-usage-report -s 24/12/01 -b A > usage.test7 &&
	grep "960" usage.test7
'

test_expect_success '-b: bank B has total usage of 300' '
	flux account view-usage-report -s 24/12/01 -b B > usage.test8 &&
	grep "300" usage.test8
'

test_expect_success '-r: bin usage by user' '
	flux account view-usage-report -s 24/12/01 -r byuser > usagebyuser.test1 &&
...
test_expect_success '-r: bin usage by bank' '
	flux account view-usage-report -s 24/12/01 -r bybank > usagebybank.test1 &&
	grep "A                                960.00" usagebybank.test1 &&
	grep "B                                300.00" usagebybank.test1
'

test_expect_success '-r: bin usage by association' '
	flux account view-usage-report -s 24/12/01 -r byassociation > usagebyassoc.test1 &&
...
test_expect_success '-t: specify usage by hour' '
	flux account view-usage-report -s 24/12/01 -u u1 -t hour > timeunit.test1 &&
	grep "0.15" timeunit.test1
'

test_expect_success '-t: specify usage by minute' '
	flux account view-usage-report -s 24/12/01 -u u1 -t min > timeunit.test2 &&
...
test_expect_success '-t: specify usage by second' '
	flux account view-usage-report -s 24/12/01 -u u1 -t sec > timeunit.test3 &&
	grep "540.00" timeunit.test3
'

test_expect_success '-S: bin by job sizes of 1,2,3,4 for u1' '
	flux account view-usage-report -s 24/12/01 -u u1 -S 1,2,3,4 > jobsizes.test1 &&
...
test_expect_success '-S: bin by job sizes of 1,2,3,4 for u2' '
	flux account view-usage-report -s 24/12/01 -u u2 -S 1,2,3,4 > jobsizes.test2 &&
...
test_expect_success '-S: bin by job sizes of 1,2,3,4 for u3' '
	flux account view-usage-report -s 24/12/01 -u u3 -S 1,2,3,4 > jobsizes.test3 &&
