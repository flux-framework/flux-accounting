...
class TestContinuousUsage(unittest.TestCase):
    ...
    # A backward wall-clock adjustment must not move the checkpoint backward or
    # make the next update apply the interval twice
    @mock.patch("time.time", mock.MagicMock(return_value=200))
    def test_04_clock_moves_backward(self):
        ...
        self.assertEqual(checkpoint, 250.0)

        with mock.patch("time.time", mock.MagicMock(return_value=251)):
            jobs.update_job_usage(conn)
        self.assertAlmostEqual(usage(conn, "user1"), before * (0.5**0.01), places=4)

    # view-user --job-usage exposes only the mirrored period-0 scalar; all older
    # periods are zero.
    @mock.patch("time.time", mock.MagicMock(return_value=251))
    def test_05_view_user_period0(self):
        breakdown = ast.literal_eval(u.view_user(conn, user="user1", job_usage=True))
        self.assertAlmostEqual(
            breakdown[0]["value"], 50.0 * (0.5**0.5) * (0.5**0.01), places=4
        )
    ...
    def test_06_cadence_invariance(self):
    ...
    def test_07_future_job_excluded(self):
    ...
    # bins.
    @mock.patch("time.time", mock.MagicMock(return_value=250))
    def test_09_edit_params_preserves_usage(self):
        before = usage(conn, "user1")
    ...
    def test_10_switch_to_periodic(self):
        before = usage(conn, "user1")
