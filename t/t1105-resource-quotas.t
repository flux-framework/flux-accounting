...
test_expect_success 'a job waiting to be scheduled is counted' '
	flux queue stop &&
	job3=$(flux submit -n1 sleep 60) &&
	flux job wait-event -t 30 ${job3} priority &&
	flux jobtap query resource_quotas.so > query.json &&
	test_debug "jq -S . <query.json" &&
	jq -e ".user_resources[\"${uid}\"].core == 1" <query.json
'

test_expect_success 'a job canceled before it runs is removed from tracked usage' '
	flux cancel ${job3} &&
...
test_expect_success 'a job waiting to be scheduled counts against the quota' '
	flux queue stop &&
	waiting=$(flux submit -N1 sleep 60) &&
	flux job wait-event -t 30 ${waiting} priority &&
	held=$(flux submit -N1 sleep 60) &&
	flux job wait-event -t 30 ${held} dependency-add &&
	flux jobs -no "{dependencies}" ${held} | grep resource-quota-user &&
	flux cancel ${waiting} &&
	flux job wait-event -t 30 ${held} dependency-remove &&
	flux job wait-event -t 30 ${held} priority &&
	flux cancel ${held} &&
	flux job wait-event -t 30 ${held} clean &&
	flux queue start &&
	flux jobtap query resource_quotas.so > query.json &&
	test_debug "jq -S . <query.json" &&
	jq -e ".user_resources == {}" <query.json &&
