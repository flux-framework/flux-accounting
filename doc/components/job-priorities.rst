...
important a job is relative to other jobs. Priorities can range from 0 to
4294967295, where the higher the number, the higher the priority, and thus,
the more important the job. Priorities are calculated as a result of a number
of different factors that all play a varying level of importance. In this
document, we will explain the factors that contribute to the calculation of a
job's priority and how to customize how job priority is calculated.
...
.. warning::
    ...
    jobs that have already completed). This is because the ``jobs`` command
    pulls the latest factor weights from the flux-accounting database, which
    could have been updated since the time a job was submitted.
