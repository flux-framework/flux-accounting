...
user
   ...
   of it a single user may have in use across their scheduled and running
   jobs. Any resource type from a jobspec may be used, including custom
   types such as ``quantum``. A type that is not in the table has no quota
   (default is empty). This table is read from the broker configuration by
   the resource quotas jobtap plugin and is not stored in the
