...
class AccountingService:
    ...
    def delete_bank(self, handle, watcher, msg, arg):
        try:
            val = b.delete_bank(
                self.conn, msg.payload.get("bank"), msg.payload.get("force")
            )
    ...
    def delete_queue(self, handle, watcher, msg, arg):
        try:
            val = qu.delete_queue(self.conn, msg.payload.get("queue"))
    ...
    def delete_project(self, handle, watcher, msg, arg):
        try:
            val = p.delete_project(self.conn, msg.payload.get("project"))
    ...
    def scrub_old_jobs(self, handle, watcher, msg, arg):
        try:
            val = jobs.scrub_old_jobs(self.conn, msg.payload.get("num_weeks"))
