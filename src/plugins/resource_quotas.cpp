 ...
 * jobspec of scheduled and running jobs, including custom types. Usage is
 * added when a job enters SCHED state, or when a held job is released, and
 * removed when the job becomes inactive. Counting at SCHED rather than RUN
 * means a job waiting for resources already holds its share of the quota,
 * so a user cannot queue up more than their quota allows and have it all
 * start at once. The tracked state can be inspected with flux jobtap query.
 *
...
// the total amount of each resource type in use by the scheduled and
// running jobs of each user, keyed by userid and then by resource type name
std::map<int, std::map<std::string, int>> user_resources;

// the ids of the jobs whose resources are currently counted in
// user_resources, so that a job is counted and released exactly once
std::set<flux_jobid_t> counted_jobs;
...
// submitted it, unless the job is already counted
static void add_resources (int userid,
                           flux_jobid_t id,
                           const std::map<std::string, int> &counts)
{
    if (!counted_jobs.insert (id).second)
        return;
    for (const auto &entry : counts)
        user_resources[userid][entry.first] += entry.second;
}

// subtract the resource totals of a counted job from the usage of the
// user that submitted it, removing entries that reach zero so the tracked
// state only contains users with scheduled or running jobs
static void remove_resources (int userid,
                              flux_jobid_t id,
                              const std::map<std::string, int> &counts)
{
    if (counted_jobs.erase (id) == 0)
        return;
 ...
 * are considered in submission order, but a job that does not fit is
 * skipped rather than blocking the jobs behind it, so a later job that
 * fits can be released ahead of an earlier one that does not. This favors
 * keeping the user's quota in use over strict first-in first-out order.
 * Since the quota is per user, the only jobs a large job can wait behind
 * are the same user's smaller jobs. The resources of a released job are
 * counted as in use immediately, so jobs released in the same pass are
 * checked against them. Returns 0 on success. Returns -1 and raises an
...
static int release_held_jobs (flux_plugin_t *p, int userid)
{
    ...
    auto job = jobs.begin ();
    while (job != jobs.end ()) {
        if (exceeds_quota (usage_of (userid), job->counts, type)) {
            ++job;
            continue;
        }
        ...
        add_resources (userid, job->id, job->counts);
        job = jobs.erase (job);
    }
    if (jobs.empty ())
        held_jobs.erase (it);
                        ...
                        void *data)
{
    ...
    if (exceeds_quota ({}, counts.counts, type))
        return flux_jobtap_reject_job (p,
                                       ...
                                       counts.get (type),
                                       type.c_str (),
                                       resource_count (user_quotas, type));
    return 0;
}
 ...
 * A job that is already past DEPEND state is counted here, since its
 * sched callback fired before this plugin was loaded. Jobs in DEPEND
 * state are handled by the depend callback, which the job manager also
 * calls for them when the plugin is loaded.
 */
                   ...
                   void *data)
{
    ...
    if (state == FLUX_JOB_STATE_SCHED
        ...
        if (get_job_resources (p, topic, args, userid, counts) < 0)
            return -1;
        add_resources (userid, id, counts.counts);
    }
 ...
 * by the scheduled and running jobs of its user would exceed a per-user
 * quota, hold it with a dependency until enough of the user's jobs finish.
 *
 * This also fires for every job in DEPEND state when the plugin is loaded.
 * The dependencies of a job are not visible to the plugin, so a job that
 * has had a dependency added is checked against the quotas again. Jobs
 * past DEPEND state are replayed first (see flux_plugin_init), so usage is
 * complete by then. If the job is over quota it is held again; adding the
 * dependency is a no-op if it is already present. If it fits, the
 * dependency is removed in case usage dropped while the plugin was not
 * loaded; removing a dependency the job does not have fails harmlessly.
 */
...
/*
 * A job has entered SCHED state and will be allocated resources as soon
 * as they are available. Add its resource totals to the usage of the user
 * that submitted it, unless it was already counted when it was released
 * from a per-user quota hold.
 */
...
/*
 * A job has become inactive. If it was counted, subtract its resource
 * totals from the usage of the user that submitted it and release any of
 * the user's held jobs that now fit under their quotas. A job that was
 * canceled while held is simply forgotten.
 */
                        ...
                        void *data)
{
    ...
    remove_resources (userid, id, counts.counts);
...
extern "C" int flux_plugin_init (flux_plugin_t *p)
{
    ...
    // replay jobs past DEPEND state before jobs in DEPEND state so that
    // usage is complete when held jobs are checked against the quotas again
