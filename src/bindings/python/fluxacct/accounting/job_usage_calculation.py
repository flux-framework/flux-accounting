...
def process_timearg(datestr):
    """
    Return seconds since the epoch for 00:00 on the date specified.
    ...
    try:
        ...
        if len(parts) == 3:
            ...
            return int(time.mktime(date.timetuple())) + 1
        raise ValueError()
    except:
        raise ValueError(
            f"unrecognized date format: {datestr}\naccepted format: YY/MM/DD"
        )
...
def view_usage_report(
    ...
    """
    if start:
        start = process_timearg(start)
    else:
        # default to grabbing jobs from the last day
        yesterday = datetime.now() - timedelta(days=1)
        start = process_timearg(yesterday.strftime("%y/%m/%d"))
    ...
    if end:
        ...
        today = datetime.now()
        end = process_timearg(today.strftime("%y/%m/%d"))
