...
def _update_job_usage_continuous(acct_conn, cur):
    ...
    wall_time = time.time()
    time_1 = max(time_0, wall_time)
    if wall_time < time_0:
        LOGGER.warning(
            "system clock moved backward; retaining usage update checkpoint at %s",
            time_0,
        )
    ...
    # usage this round and just anchors the checkpoint at t1. time_1 always moves in one
    # direction, so a backwards will not move the checkpoint backwards
