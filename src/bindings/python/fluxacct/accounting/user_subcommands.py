...
def add_user(
    ...
    # check if user/bank entry already exists but was deleted first; if so,
    # just update the 'deleted' column in already existing row
    cur.execute(
    ...
    if len(rows) == 1:
        cur.execute(
            "UPDATE association_table SET deleted=0 WHERE username=? AND bank=?",
            (
                username,
                bank,
            ),
